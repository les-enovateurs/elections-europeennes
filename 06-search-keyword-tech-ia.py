...
with open(
    "./data/refacto/programme-website-stats-falc.json", "r", encoding="utf-8"
) as file:
    data = json.load(file)
...
for entry in data["data"]:
    ...
    if entry["pdf_acc"] != "0":
        occurenceTechKeyword = find_keyword(
            entry["numPanneau"], entry["pdf_acc"], keywordTech
        )
        if occurenceTechKeyword:
            entry["hasTech"] = {"name": "pdf_acc", "occurence": occurenceTechKeyword}

    if entry["hasTech"] == 0 and entry["falc_acc"] != "0":
        occurenceTechKeyword = find_keyword(
            entry["numPanneau"], entry["falc_acc"], keywordTech
        )
        if occurenceTechKeyword:
            entry["hasTech"] = {"name": "falc_acc", "occurence": occurenceTechKeyword}

    if entry["hasTech"] == 0 and entry["pdf"] != "0":
        occurenceTechKeyword = find_keyword(
            entry["numPanneau"], entry["pdf"], keywordTech
        )
        if occurenceTechKeyword:
            entry["hasTech"] = {"name": "pdf", "occurence": occurenceTechKeyword}

    if entry["hasTech"] == 0 and entry["falc"] != "0":
        occurenceTechKeyword = find_keyword(
            entry["numPanneau"], entry["falc"], keywordTech
        )
        if occurenceTechKeyword:
            entry["hasTech"] = {"name": "falc", "occurence": occurenceTechKeyword}

    # Ajouter l'entrée mise à jour à la nouvelle liste
    updated_data.append(entry)

# Nouveau dictionnaire avec les données mises à jour
updated_json = {"data": updated_data}
...
with open(
    "data/refacto/programme-website-stats-falc-keyword-tech.json", "w", encoding="utf-8"
) as f:
