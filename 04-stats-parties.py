...
for partie, partie_data in df.groupby("Numéro de panneau"):
    ...
    partie_data["Profession"] = partie_data["Profession"].str.replace(
        pattern, "", regex=True
    )
    ...
    plt.savefig(
        os.path.join(dossier_partie, f"camembert_professions_partie.svg"),
        format="svg",
        dpi=300,
    )
    plt.close()
    ...
    with open(
        os.path.join(dossier_partie, f"repartition_professions_partie.json"), "w"
    ) as json_file:
    ...
    # plt.savefig(os.path.join(dossier_partie, f'camembert_professions_partie.png'))
    plt.savefig(
        os.path.join(dossier_partie, f"camembert_parite.svg"), format="svg", dpi=300
    )
    plt.close()
    ...
    with open(
        os.path.join(dossier_partie, f"repartition_parite.json"), "w"
    ) as json_file:
    ...
    partie_data["Date de naissance"] = pd.to_datetime(
        partie_data["Date de naissance"], format="%d/%m/%Y"
    )

    # Calculer l'âge à partir de la date de naissance
    partie_data["Age"] = (
        pd.Timestamp.now().year - partie_data["Date de naissance"].dt.year
    )
    ...
    partie_data["Tranche d'âge"] = pd.cut(
        partie_data["Age"], bins=bins, labels=labels, right=False
    )
    ...
    plt.savefig(
        os.path.join(dossier_partie, f"camembert_tranches_age_partie.svg"),
        format="svg",
        dpi=300,
    )
    plt.close()
    ...
    with open(
        os.path.join(dossier_partie, f"repartition_tranches_age.json"), "w"
    ) as json_file:
